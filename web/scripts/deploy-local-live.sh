#!/usr/bin/env bash
set -euo pipefail
# Deploy local (Node + Route Handlers). Contrato de portas e URLs: ../../docs/03_operacao_e_deploy.md
...
if [[ "${SKIP_PORT_CHECK:-0}" != "1" ]] && lsof -i ":$PORT" -sTCP:LISTEN >/dev/null 2>&1; then
  echo "Port $PORT in use — see ../../docs/03_operacao_e_deploy.md (free the port or SKIP_PORT_CHECK=1 as exception)." >&2
  exit 1
fi
...
if [[ "${SKIP_BUILD:-0}" == "1" ]]; then
  ...
  echo "→ build:server (learnchinese.today + Route Handlers /api)…"
  npm run build:server
fi

echo ""
echo "  Site + API:  http://127.0.0.1:${PORT}/"
echo "  Tutor:       http://127.0.0.1:${PORT}/tutor"
